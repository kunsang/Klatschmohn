 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
class WP_ImportDEMO extends WP_Importer {
    ...
    function update_sub_status($progress = 0)
    {
        $position = $this->progress;
        if ($this->progressmax)
        {
            $position += ceil(($this->progressmax - $this->progress) * $progress / 100);
        }
        do_action('action_import_update_status', $this->progress_message, $position);
    }    
    
    function update_status($message = '', $progress = 0)
    {
        if ($message)
        {
        ...
        do_action('action_import_update_status', $this->progress_message, $this->progress);
        if ($progress)
        {   
    ...
    function WP_ImportDEMO() { /* nothing */ }
    ...
    function import( $file ) {
        ...
        $this->update_status('Import Start', 5);
        $this->import_start( $file );
        $this->get_author_mapping();
        wp_suspend_cache_invalidation( true );
        $this->update_status('Import Categories', 10);
        $this->process_categories();
        $this->update_status('Import Tags', 15);
        $this->process_tags();
        $this->update_status('Import Terms', 20);
        $this->process_terms();
        $this->update_status('Import Posts', 85);
        $this->process_posts();
        wp_suspend_cache_invalidation( false );
        // update incorrect/missing information in the DB
        $this->backfill_parents();
        $this->update_status('Import Attachments', 95);
        $this->backfill_attachment_urls();
        $this->remap_featured_images();
        $this->import_end();
        $this->update_status('Import Custom Configuration', 100);
        ...
        foreach ($menus as $menu){
            ...
            $thememod[$slug] = $menu->term_id;
        }
        if (count($thememod) > 0)
        {
            set_theme_mod('nav_menu_locations', $thememod);
        }        
        ...
        $this->update_status('Import Completed');
    }                       
...
<?php
    ...
    function process_posts() {
        foreach ( $this->posts as $post ) {
            ...
            $this->update_sub_status(ceil(count($this->processed_posts) * 100 / count($this->posts)));
        }
    ...
    function fetch_remote_file( $url, $post ) {
        ...
        // Try to find the file on the local computer
        $headers = array();
        if (file_exists(HONEY_PATH_TO_THEME_CODE.'/demo/images/'.$file_name)) 
        {   
            file_put_contents($upload['file'],  file_get_contents(HONEY_PATH_TO_THEME_CODE.'/demo/images/'.$file_name));
        } else                           
        {
